package com.dream11.logcentralorchestrator.dao.query;
...
public class Query {
  public final String GET_SERVICES =
      "SELECT env, serviceName, componentName, retentionDays, tenant FROM service_details WHERE tenant = ?;";
  public final String INSERT_SERVICE_DETAILS =
      "INSERT INTO service_details (env, serviceName, componentName, retentionDays, tenant) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE lastCheckedAt = NOW()";
  public final String DELETE_SERVICE_DETAILS =
      "DELETE FROM service_details WHERE env = ? AND serviceName = ? AND componentName = ? AND tenant = ?;";
