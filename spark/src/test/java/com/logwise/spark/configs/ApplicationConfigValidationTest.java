 ...
 *
 * <p>
 * Tests verify that configuration loading works correctly with valid and
 * invalid inputs, and
 * that configuration precedence (command line args > system properties > file)
 * is respected.
 */
public class ApplicationConfigValidationTest {
    ...
    public void testGetConfig_WithSystemProperties_OverridesFileConfig() {
        ...
        try {
            ...
            Assert.assertTrue(config.hasPath("app.job.name") || config.hasPath("kafka.bootstrap.servers.port"));
        } finally {
            // Cleanup
