 ...
 *
 * <p>Tests verify handling of edge cases, boundary conditions, and unusual scenarios in Kafka
 * service operations.
 */
public class KafkaServiceEdgeCaseTest {
  ...
  public void testGetStartingOffsetsByTimestamp_WithVeryOldTimestamp_ReturnsEmpty() {
    ...
    StartingOffsetsByTimestampOption result =
        kafkaService.getStartingOffsetsByTimestamp(LOCALHOST, "logs.*", veryOldTimestamp);
    ...
    Assert.assertTrue(
        result.getOffsetByTimestamp().isEmpty() || result.getOffsetByTimestamp().size() == 0);
  }
  ...
  public void testGetStartingOffsetsByTimestamp_WithFutureTimestamp_HandlesGracefully() {
    ...
    // Act
    StartingOffsetsByTimestampOption result =
        kafkaService.getStartingOffsetsByTimestamp(LOCALHOST, "logs.*", futureTimestamp);
  ...
  public void testGetStartingOffsetsByTimestamp_WithEmptyTopicPattern_ReturnsEmpty() {
    ...
    // Act
    StartingOffsetsByTimestampOption result =
        kafkaService.getStartingOffsetsByTimestamp(LOCALHOST, "", System.currentTimeMillis());
  ...
  public void testGetStartingOffsetsByTimestamp_WithWildcardPattern_MatchesAllTopics() {
    ...
    Map<String, List<PartitionInfo>> topicsMetadata = new HashMap<>();
    topicsMetadata.put(
        "logs-topic1", Collections.singletonList(createPartitionInfo("logs-topic1", 0)));
    topicsMetadata.put(
        "logs-topic2", Collections.singletonList(createPartitionInfo("logs-topic2", 0)));
    topicsMetadata.put(
        "other-topic", Collections.singletonList(createPartitionInfo("other-topic", 0)));

    Map<TopicPartition, OffsetAndTimestamp> offsets = new HashMap<>();
    offsets.put(
        new TopicPartition("logs-topic1", 0),
        new OffsetAndTimestamp(100L, System.currentTimeMillis()));
    offsets.put(
        new TopicPartition("logs-topic2", 0),
        new OffsetAndTimestamp(200L, System.currentTimeMillis()));
