...
import com.dream11.logcentralorchestrator.rest.MonitorSparkJob;
...
public class MonitorSparkJobTest {
  ...
  public void setUp() throws Exception {
    ...
    // Since MonitorSparkJob uses @RequiredArgsConstructor with final fields,
  ...
  public void testHandle_WithNullRequest_ReturnsSuccessResponse() throws Exception {
    // Arrange
    MonitorSparkJobRequest request = null; // null request should use defaults
    ...
    verify(sparkService, times(1)).monitorSparkJob(eq(Tenant.D11_Prod_AWS), isNull(), isNull());
  }
  ...
  public void testHandle_WithOnlyDriverCores_ReturnsSuccessResponse() throws Exception {
    ...
    // Verify service was called with correct values
    verify(sparkService, times(1))
        .monitorSparkJob(eq(Tenant.D11_Prod_AWS), eq(TestConstants.DEFAULT_DRIVER_CORES), isNull());
  }
  ...
  public void testHandle_WithOnlyDriverMemory_ReturnsSuccessResponse() throws Exception {
    ...
    verify(sparkService, times(1))
        .monitorSparkJob(
            eq(Tenant.D11_Prod_AWS), isNull(), eq(TestConstants.DEFAULT_DRIVER_MEMORY_GB));
  }
}
